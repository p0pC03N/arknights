...
function BreakingNewsTag({ label, active, onClick }: {
    ...
    return (
        <div
            onClick={onClick}
            className={
                (active ? "text-black bg-ark-blue " : "hover:text-ark-blue ") +
...
function BreakingNewsList() {
    ...
    const currentName = data[categoryIndex]?.name ?? ""
...
function SwiperInfo({ swiperIndex }: { swiperIndex: number }) {
    ...
    return (
        <div className="w-[26.5rem] portrait:w-[unset] portrait:static absolute left-[3.875rem] bottom-[-1.875rem]">
            <div className={"transition-[opacity,transform] ease-in-out duration-150 " + animeClass}>
                ...
                >
                    <div>
                        <div className={"text-[1.25rem] font-black"}>更多内容</div>
    ...
    return (
        <div className="w-full h-full relative">
            <div className="w-full h-full overflow-hidden absolute">
                <div
                    className="w-full h-full bg-center bg-cover bg-no-repeat blur"
...
export default function Information() {
    ...
    const currentTabLabel =
        swiperIndex >= 0 ? undefined : undefined

    return (
        <div className="w-[100vw] max-w-[180rem] h-full absolute top-0 right-0 bottom-0 left-auto transition-opacity duration-100">
            <PortraitBottomGradientMask />
            <SwiperBody setSwiperIndex={setSwiperIndex} />
            <SwiperScrollbar />
...
function DynamicSectionTitle() {
    ...
    useEffect(() => {
        const handleCategorySync = (event: Event) => {
            const customEvent = event as CustomEvent<number>
            setCategoryIndex(customEvent.detail ?? 0)
        }

        window.addEventListener("information-category-change", handleCategorySync)
        return () => window.removeEventListener("information-category-change", handleCategorySync)
    }, [])

    const title = data[categoryIndex]?.name === "文档" ? "DOCS" : "BLOG"
...
function BreakingNewsWithTitle() {
    ...
    useEffect(() => {
        window.dispatchEvent(new CustomEvent("information-category-change", { detail: categoryIndex }))
    }, [categoryIndex])
